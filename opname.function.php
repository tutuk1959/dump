<?php
	...
	function _validation($toko, $tanggalOpname,&$errors){
		enarray($errors);
		text($errors, $toko, 'Tentukan Toko Produk, Toko tidak boleh kosong!', 100, REQUIRED);
		text($errors, $tanggalOpname, 'Tanggal Opname tidak boleh kososng!', 10, REQUIRED);
	}
	
	function kickView($hakAkses){
		if (!in_array('VIEW_OPNAME', $hakAkses)){?>
			<script>
	...
	function kickInsert($hakAkses){
		if (!in_array('INSERT_OPNAME', $hakAkses)){?>
			<script>
				location = 'dashboard.php';
				alert('Anda tidak memiliki hak akses!');
			</script>
	...
	function kickManage($hakAkses){
		if (!in_array('MANAGE_OPNAME', $hakAkses)){?>
			<script>
				location = 'dashboard.php';
				alert('Anda tidak memiliki hak akses!');
			</script>
