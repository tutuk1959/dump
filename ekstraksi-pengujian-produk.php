...
if ($_REQUEST['mode'] == 'insert' && $_REQUEST['submit'] == 'Ekstrak') {
	...
	if (!$error){
		...
		$idProduk = $db->row("SELECT a.idProduk,a.idSize FROM pelatihan AS a WHERE a.idPelatihan = '%s'",$_REQUEST['idPel']);
		foreach ($db->table("SELECT a.`idProduk`, LEFT(c.`tanggalOpname`, 7) bulan, Year(c.`tanggalOpname`) ye,  Month(c.`tanggalOpname`) mo, 
				...
				WHERE c.idOpname IS NOT NULL AND a.idProduk = '%s' AND b.idSize = '%s'
				GROUP BY a.`idProduk`, LEFT(c.`tanggalOpname`, 7)",$tanggalDari, $tanggalSampai,$idProduk->idProduk,$idProduk->idSize) as $row)
			$data[$row->idProduk.".".($row->ye *12 + $row->mo)] = $row;
		...
		foreach ($db->table("SELECT a.`idProduk`, LEFT(c.`tanggalOpname`, 7) bulan, Year(c.`tanggalOpname`) ye,  Month(c.`tanggalOpname`) mo, 
				...
				LEFT JOIN `opname` c ON b.`idOpname`=c.`idOpname` AND c.`tanggalOpname` BETWEEN '%s' AND '%s'
				WHERE c.idOpname IS NOT NULL AND a.idProduk = '%s' AND b.idSize = '%s'
				GROUP BY a.`idProduk`, LEFT(c.`tanggalOpname`, 7)",$tanggalKemarinAwal,$tanggalKemarinAkhir,$idProduk->idProduk,$idProduk->idSize) as $row)
			if (isset($data[$row->idProduk.".".($row->ye *12 + $row->mo + 1)])){
				$data[$row->idProduk.".".($row->ye *12 + $row->mo + 1)]->terjualLalu = $row->terjuallalu;
			}
		//$data = array(
...
if ($_REQUEST['mode'] == 'insert' && $_REQUEST['simpan'] == 'Simpan'){
	$db->exec("DELETE FROM pengujian WHERE idPelatihan = '%s'",$_REQUEST['idPelatihan']);
	$db->exec("INSERT INTO pengujian(idPelatihan) VALUES ('%s')",$_REQUEST['idPelatihan']);
	$idPengujian = $db->insertID();
	$db->exec("DELETE FROM inputpengujian WHERE idPelatihan = '%s'",$_REQUEST['idPelatihan']);
	foreach($_REQUEST['bulan'] as $k=>$v){
		...
		$_REQUEST['stokAwal'][$k], $_REQUEST['terjual'][$k], $error);
		if (! $error){
			$db->exec("INSERT INTO inputpengujian(idPelatihan,idPengujian, x1,x2,x3,y) VALUES ('%s','%s','%s','%s','%s','%s')",
				...
				<div class="col-lg-12">
					...
					<div class="nav-tabs-custom">
						<ul class="nav nav-tabs">
							<ul class="nav nav-tabs">
							<li ><a href="parameter-pengujian-produk.php?produk=<?=$_REQUEST['idProduk'];?>&size=<?=$_REQUEST['idSize'];?>&outlet=<?=$_REQUEST['idOutlet']?>">Parameter</a></li>
							<li class="active"><a href="#ekstraksi-pelatihan" data-toggle="tab">Ekstraksi</a></li>
							<li><a  href="hasil-pengujian-produk.php?idPelatihan=<?=($_REQUEST['idPelatihan']) ? $_REQUEST['idPelatihan'] : ''?>&idPengujian=<?=$idPengujian?>">Hasil</a></li>
						</ul>
						</ul>
							...
							<div class="active tab-pane" id="ekstraksi-pelatihan">
								<div class="box box-info">
									<div class="box-header with-border">
										<h3 class="box-title">Ekstraksi dari data Opname berdasarkanProduk</h3>
									</div>
													...
													<tbody>
														...
														<?php foreach((array)$normalized as $k=>$v):?>
															...
															<tr>
																...
																<td>
																	<input type="hidden" name="hargaNorm[]" value="<?=($v->harga == NULL) ? htmlentities($data[$k]->harga) : htmlentities($v->harga);?>"/>
												...
												<div class="box-footer">
													...
													<?php if ($normalized):?>
														<div class="pull-right">
															...
															<a href="backprop-pengujian-produk.php?idPelatihan=<?=$_REQUEST['idPelatihan'];?>&idPengujian=<?=$idPengujian;?>" target="_blank"><button type="button" value="Proses"class="btn btn-info">Proses</button></a>
														</div>
