<?php
	...
	function _validation($jenisproduk,&$errors){
		enarray($errors);
		text($errors, $jenisproduk, 'Field Jenis Produk tidak boleh kosong! (Maksimal 100 karakter!)', 100, REQUIRED);
	}
	
	function kickView($hakAkses){
		if (!in_array('VIEW_JENIS', $hakAkses)){?>
			<script>
	...
	function kickInsert($hakAkses){
		if (!in_array('INSERT_JENIS', $hakAkses)){?>
			<script>
				location = 'dashboard.php';
				alert('Anda tidak memiliki hak akses!');
			</script>
	...
	function kickManage($hakAkses){
		if (!in_array('MANAGE_JENIS', $hakAkses)){?>
			<script>
				location = 'dashboard.php';
				alert('Anda tidak memiliki hak akses!');
			</script>
