<?php
	...
	function _validation($toko, $tanggalSupply,&$errors){
		enarray($errors);
		text($errors, $toko, 'Tentukan Toko atau Outlet, Toko tidak boleh kosong!', 100, REQUIRED);
		text($errors, $tanggalSupply, 'Tanggal Pasok tidak boleh kosong!', 10, REQUIRED);
	}
	
	function kickView($hakAkses){
		if (!in_array('VIEW_PASOK', $hakAkses)){?>
			<script>
	...
	function kickInsert($hakAkses){
		if (!in_array('INSERT_PASOK', $hakAkses)){?>
			<script>
				location = 'dashboard.php';
				alert('Anda tidak memiliki hak akses!');
			</script>
	...
	function kickManage($hakAkses){
		if (!in_array('MANAGE_PASOK', $hakAkses)){?>
			<script>
				location = 'dashboard.php';
				alert('Anda tidak memiliki hak akses!');
			</script>
